...
export const News: React.FC<NewsProps> = ({ limit, isPage = false }) => {
  ...
  const filteredItems = useMemo(() => {
    ...
    if (isPage && activeFilter !== t.news.newsTags[0]) {
      items = items.filter((item: any) => item.tag === activeFilter);
    }
    return limit ? items.slice(0, limit) : items;
  }, [t.news.items, activeFilter, limit, isPage, t.news.newsTags]);
        ...
        {isPage ? (
            ...
            {filteredItems.map((item: any, idx: number) => (
              ...
              >
                <div className="relative h-64 overflow-hidden bg-slate-100">
                  <img src={images[idx % images.length]} alt={item.title} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <article 
              ...
              onClick={() => setActiveNews(filteredItems[0].id)}
            >
              <div className="absolute inset-0">
                <img src={images[0]} alt={filteredItems[0].title} className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-105 opacity-60" />
              ...
              <div className="relative p-12 text-white w-full z-10">
                <div className="flex items-center gap-4 mb-6">
                  <span className="bg-emerald text-navy text-[10px] font-black uppercase tracking-widest px-4 py-2 rounded-full">{filteredItems[0].tag}</span>
                  <time className="text-white/80 text-xs font-bold uppercase tracking-widest">{filteredItems[0].date}</time>
                </div>
                <h3 className="text-3xl md:text-5xl font-extrabold mb-6 tracking-tighter leading-[1.1]">{filteredItems[0].title}</h3>
                <p className="text-white/90 text-lg font-light mb-8 max-w-xl">{filteredItems[0].desc}</p>
            ...
            <div className="lg:col-span-5 flex flex-col gap-8">
              {filteredItems.slice(1, 3).map((item: any, idx: number) => (
                <article 
                  key={item.id} 
                  onClick={() => setActiveNews(item.id)}
